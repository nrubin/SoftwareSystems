...
void setup() {
  ...
  attachInterrupt(0, wave0Select, RISING);  // Interrupt attached to the button connected to pin 2
  attachInterrupt(1, wave1Select, RISING);  // Interrupt attached to the button connected to pin 3
...
void writeByte(int x) {
  ...
  PORTB = x >> 2;   // turns on 8..13
}

ISR(TIMER1_COMPA_vect) {
  writeByte(waveformsTable[selectedWave][i]);
  i = (i+1) % maxSamplesNum;
}
...
void loop() {
  Serial.print(waveformsTable[selectedWave][i]);
  Serial.print(", ");
  Serial.println(selectedWave);
  delay(400);
}
